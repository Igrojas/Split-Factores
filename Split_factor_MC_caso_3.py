...
for i in range(iter_mc):
    for sim in lista_equipos:
        ...
        sp_jms_1    =  [round(random.uniform(0.01, 0.95), 2), round(random.uniform(0.01, 0.85), 2)]
...
df_resultados.to_excel(os.path.join('results', 'results_caso_3.xlsx'), index=False)
...
def graficar_resultados(df, rec_min=None, rec_max=None, ley_min=None, ley_max=None, solo_flujo9_cut=False):
    ...
    if solo_flujo9_cut:
        # Filtrar solo por 'Flujo 9 Cut' > 0.2 y < 30
        if 'Flujo 9 Cut' in df_filtrado.columns:
            df_filtrado = df_filtrado[df_filtrado['Flujo 9 Cut'] <= 0.26]
    ...
    df_filtrado = df_filtrado[df_filtrado['er_jameson_1'] <= 5]
...
df_filtrado = graficar_resultados(df_resultados.round(2), rec_min=90, ley_min=10, ley_max=30)
# Ejemplo de uso SOLO filtrando por Flujo 9 Cut:
# df_filtrado = graficar_resultados(df_resultados.round(2), rec_min=90, ley_min=10, ley_max=30, solo_flujo9_cut=True)

def guardar_resultados_avanzado(df_filtrado, archivo='results/results_caso_3_filtered.xlsx'):
    """
